...
auto evaluate_extraction_of_a_root(std::stack<double>& stack) -> void
{
    if (stack.size() < 1) {
        throw std::logic_error{"not enough operands for -"};
    }
...
auto evaluate_round_multiplication(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for -"};
    }
    ...
    for (auto const each : args) {
        try {
            if (each == "p") {
                ...
                evaluate_round_multiplication (stack);
            } else {
